...
3. bundled leg authority for lower-body topology when legs are visible
4. bundled canonical Xavier reference and approved official frames for all other identity fields
5. original-show imagery for base construction and show-adjacent mood
6. official or creator-connected material for behavior and story grammar
7. verified community practice for remix grammar
8. inspected original-platform PS2 screenshots for environment and rendering only
...
Never redraw, enhance, crop, denoise, upscale, re-light, re-encode, or silently replace either bundled authority asset. When references are supported and the legs are visible, attach both assets and state their separate roles.
...
Before multi-shot work, create a compact Scene Lock and a Genesis Frame. Check identity, leg construction when visible, rendering, premise causality, geography, and rights. A leg-visible frame with standard human knees, animal hocks, extra segments, or ambiguous joint direction fails the gate. One narrow automatic repair is allowed for an isolated failure. Do not expand a failed Genesis Frame.

Treat `approved`, `perfect`, `lock it`, and clear equivalents as approval. After approval, preserve identity, leg rig, geography, screen direction, props, action state, lighting, texture ceiling, and dialogue/caption state. A later shot changes only what the story changes.
